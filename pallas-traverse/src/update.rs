...
// The second form takes a further variant list whose arms are compiled only
// with the `unstable` feature, which is how an era behind that gate reaches a
// generated accessor. A variant cannot be gated inside the list itself, and
// leaving it out would have the catch-all answer for it.
