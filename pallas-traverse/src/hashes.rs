...
mod tests {
    ...
    /// Every fixture's block hash, header hash, block number and slot against
    /// the values the node itself recorded.
    ///
    /// These are the only node produced hashes in the repository. Each was
    /// checked against the node's secondary index when the block was cut, and
    /// the provenance file beside the fixtures records them, so this is the
    /// one place where the crate is checked against something other than
    /// itself.
    #[test]
    fn every_fixture_block_hashes_as_the_node_recorded_it() {
        ...
        // The name of this test claims every fixture, so the number it
        // reaches is stated. `pallas-primitives` is where a new fixture
        // without a recorded hash fails, because the list of fixtures and the
        // list of hashes live beside each other there and are compared.
        #[cfg(feature = "unstable")]
        assert_eq!(cases.len(), 14, "one Conway fixture and thirteen Dijkstra");
    ...
    /// The chain links across the fork: the first Dijkstra block names the
    /// last Conway one as its parent. A header hash rule that was wrong for
    /// one of the two eras could not produce this.
    #[cfg(feature = "unstable")]
    #[test]
    ...
    /// Three Dijkstra transaction ids against the node's own UTxO set.
    ///
    /// A node keys an unspent output by the id it computed for the
    /// transaction that made it. Each id below was put to a Musashi node over
    /// a local state query and came back with the output still unspent, which
    /// is the node agreeing that this is the transaction's id. The other eight
    /// ids in these fixtures had been spent by then and prove nothing either
    /// way, so they are not asserted here.
    #[cfg(feature = "unstable")]
    #[test]
    ...
    /// The auxiliary data hash a Dijkstra transaction body commits to at key
    /// 7, recomputed from the auxiliary data the same transaction carries.
    ///
    /// Four transactions across three fixtures declare the key. Nothing else
    /// in the crate could recompute this for the era before, because there was
    /// no hash implementation for this era's auxiliary data type.
    #[cfg(feature = "unstable")]
    #[test]
