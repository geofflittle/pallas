...
/// setting one `protocol_param_update` key.
///
/// No fixture on this chain carries a proposal, so these bytes were built
/// rather than read. They live beside the block fixtures because
/// `pallas-traverse` reads the same two files: a proposal is the only
/// thing that carries a parameter update, and two crates building it
/// separately can drift apart with nothing to notice.
