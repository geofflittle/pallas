...
impl<C: LedgerContext> Mapper<C> {
    ...
    /// Maps the per-tx voting procedures into the v1beta `votes` field.
    ///
    /// Conway and Dijkstra share the voting procedure type, so both eras
    /// answer here.
...
mod tests {
    ...
    /// A Dijkstra transaction's votes reach u5c. The two eras share the voting
    /// procedure type, so nothing but the gate stood between them.
    #[cfg(feature = "unstable")]
    #[test]
    ...
    fn snapshot() {
        ...
        cases.push((
            ...
            include_str!("../../../test_data/u5c_v1beta_dijkstra_map_output.json"),
            "u5c_v1beta_dijkstra_map_output.json",
        ));
